...
void cellwritedynamic(char* cellfile, int** cell, int l)
{
  ...
  printf("cellwritedynamic: opening file <%s>\n", cellfile);

  fp = fopen(cellfile, "w");

  printf("cellwritedynamic: writing data ...\n");
  ...
  fprintf(fp, "P1\n");
  fprintf(fp, "# Written by cellwritedynamicdynamic\n");
  fprintf(fp, "%d %d\n", l, l);
  ...
  printf("cellwritedynamic: ... done\n");

  fclose(fp);
  printf("cellwritedynamic: file closed\n");
}